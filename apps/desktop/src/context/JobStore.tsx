...
const isTerminalStatus = (
  ...
  JobState,
  "completed" | "failed" | "canceled" | "partial_success"
> =>
  status === "completed" ||
  status === "failed" ||
  status === "canceled" ||
...
const buildTranslationFilesPayload = (
  ...
  let filesPayload = (job.files ?? [])
    ...
    .map((file) => ({
      ...
      ...(file.archivePath && { archivePath: file.archivePath }),
      ...(file.archivePath && { archiveEntryPath: file.path }),
    }));
...
export function JobStoreProvider({ children }: { children: ReactNode }) {
  ...
  const finalizeCurrentJob = useCallback(
    (
      ...
      setState((prev) => {
        ...
        const existingIndex = prev.completedJobs.findIndex(
          (job) => job.id === finishedJob.id,
        );
        const completedJobs =
          existingIndex === -1
            ? [...prev.completedJobs, finishedJob]
            : prev.completedJobs.map((job, index) =>
                index === existingIndex ? finishedJob : job,
              );
  ...
  const appendLog = useCallback(
    (message: string, level: JobLogLevel = "info") => {
      ...
      setState((prev) => {
        if (prev.currentJob && prev.currentJob.modId === input.modId) {
          outcome = {
            job: prev.currentJob,
            promoted: false,
            error: "duplicate-active",
          };
          return prev;
        }

        const duplicateQueued = prev.queue.find(
          (job) => job.modId === input.modId,
        );
        if (duplicateQueued) {
          outcome = {
            job: duplicateQueued,
            promoted: false,
            error: "duplicate-queued",
          };
          return prev;
        }
    ...
    try {
      ...
      setState((prev) => {
        ...
        const targetLanguage =
          prev.currentJob.targetLanguage ?? DEFAULT_TARGET_LANGUAGE;
  ...
  const startTranslationForCurrentJob = useCallback(
    async (options: StartTranslationOptions) => {
      ...
      const sourceLanguage =
        options.sourceLanguageGuess ?? activeJob.sourceLanguageGuess;

      const filesPayload = buildTranslationFilesPayload(
        activeJob,
    ...
    if (!isTauri()) {
      if (previousStatus === "pending") {
        const finalLog = createLogEntry(
          "warn",
          "User canceled while preparing.",
        );
        finalizeCurrentJob("canceled", finalLog, {
      ...
      if (status === "running" || status === "pending") {
        setState((prev) => {
          ...
          const containsSensitiveData =
            ...
            trimmedLog.includes(providerKey);
          ...
          const updatedFileErrors = applyFileErrorUpdates(
            prev.currentJob,
            payload,
          );
          ...
          const nextResumeHint = resumePayload
            ? {
                filePath: resumePayload.filePath,
                lineNumber: resumePayload.lineNumber,
              }
            : prev.currentJob.resumeHint;
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        console.error(
          "translation-progress 이벤트 등록에 실패했습니다.",
          error,
        );
      }

      try {
        const unlistenBackoffStarted =
          await listen<TranslationBackoffStartedPayload>(
        ...
        console.error(
          "translation-backoff-started 이벤트 등록에 실패했습니다.",
          error,
        );
      }

      try {
        const unlistenBackoffCancelled =
          await listen<TranslationBackoffCancelledPayload>(
        ...
        console.error(
          "translation-backoff-cancelled 이벤트 등록에 실패했습니다.",
          error,
        );
      }

      try {
        const unlistenRetryStarted =
          await listen<TranslationRetryStartedPayload>(
        ...
        console.error(
          "translation-retry-started 이벤트 등록에 실패했습니다.",
          error,
        );
      }
    };
...
export function useJobStore() {
  ...
  if (!context) {
    throw new Error(
      "JobStore는 JobStoreProvider 내부에서만 사용할 수 있습니다.",
    );
  }
  return context;
}

export function getDefaultJobRoute(
...
export const providerLabelFor = (provider: ProviderId) =>
