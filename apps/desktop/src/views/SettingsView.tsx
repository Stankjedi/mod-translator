...
type ProviderRetryPolicy =
  import("../storage/settingsStorage").ProviderRetryPolicy;
...
function SettingsView() {
  ...
  const formatDetectedNote = useMemo(
    () => (path: string) => t("settings.steam.noteDetected", { path }),
    [t],
  );
  const {
    steamPath,
  ...
  const activeProviderKey = activeProviderId
    ? (apiKeys[activeProviderId] ?? "")
    : "";
  const activeProviderKeyMissing =
    Boolean(activeProviderId) && !activeProviderKey.trim();
  const activeProviderName = useMemo(
    () =>
      providers.find((provider) => provider.id === activeProviderId)?.name ??
  ...
  useEffect(() => {
    ...
    if (trimmed) {
      ...
      setPathNote(t("settings.steam.noteNotFound"));
    }
  }, [steamPath, formatDetectedNote, t]);
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      if (detected) {
        ...
        setScanStatus(t("settings.steam.scanning"));
        try {
          await scanLibrary(detected);
          setScanStatus(t("settings.steam.noteDone"));
        } catch (error) {
          console.error("failed to scan detected Steam path", error);
          setScanStatus(t("settings.steam.noteError"));
        }
      } else {
        setPathNote(t("settings.steam.noteNotFound"));
        setScanStatus("");
      ...
      setPathNote(t("settings.steam.noteError"));
      setScanStatus(t("settings.steam.noteError"));
    }
  };
  ...
  const handleScan = async () => {
    ...
    if (!trimmed) {
      setPathNote(t("settings.steam.noteEmpty"));
      setScanStatus("");
      return;
    }
  ...
  const handleApiKeySave = useCallback(
    (providerId: string, providerName: string) => {
      ...
      try {
        ...
        setApiKeyError(
          "API 키 저장에 실패했습니다. 저장소 접근 권한을 확인하세요.",
        );
      }
    },
  ...
  const handleApiKeyRemove = useCallback(
    (providerId: string, providerName: string) => {
      try {
        ...
        setApiKeyError(
          "API 키를 제거하는 중 문제가 발생했습니다. 다시 시도해 주세요.",
        );
      }
    },
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-slate-400">
          번역 엔진, Steam 연동, 그리고 처리량 제한을 조정할 수 있습니다. 이
          화면은 Rust 백엔드와의 연계를 고려한 자리 표시자 UI입니다.
        </p>
      </header>
      ...
      <form className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-800/60 bg-slate-900/60 p-6 shadow-inner shadow-black/30">
          ...
          <p className="mt-1 text-sm text-slate-400">
            각 제공자별 API 키를 직접 입력해 Rust 백엔드와의 연동을 준비하세요.
            빈 값으로 저장하면 키가 제거됩니다.
          </p>
          {activeProviderKeyMissing && (
            <div className="mt-4 rounded-xl border border-rose-500/40 bg-rose-500/10 px-4 py-3 text-xs text-rose-200">
              {activeProviderName} API 키가 설정되지 않았습니다. 번역 작업을
              예약하기 전에 키를 입력하세요.
            </div>
          )}
          <div className="mt-4 space-y-4">
            {providers.map((provider) => {
              ...
              const fallbackModels = modelOptions.filter(
                (model) => !verifiedList.includes(model),
              );
                ...
                discovery.source === "live"
                  ...
                  : discovery.networkError
                    ? {
                        text: "네트워크 오류",
                        className:
                          "bg-rose-700/70 text-rose-100 border-rose-400/40",
                      }
                    : {
                        text: "대체 목록",
                        className:
                          "bg-amber-700/70 text-amber-100 border-amber-400/40",
                      };
              const dropdownNote = (() => {
                if (checking) {
              ...
              const handleRetryNumberChange =
                (field: RetryNumericField) =>
                (event: ChangeEvent<HTMLInputElement>) => {
                  const value = Number(event.target.value);
                  setProviderRetryPolicy(provider.id, {
                    [field]: value,
                  } as Partial<ProviderRetryPolicy>);
                };
              const handleRetryToggleChange =
                (field: RetryBooleanField) =>
                (event: ChangeEvent<HTMLInputElement>) => {
                  setProviderRetryPolicy(provider.id, {
                          ...
                          <>
                            ...
                            <div className="flex gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  handleApiKeySave(provider.id, provider.name)
                                }
                            ...
                            <div className="flex items-center gap-3">
                              <div className="flex gap-2">
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleStartEditing(provider.id)
                                  }
                  ...
                  <div className="mt-5 rounded-xl border border-slate-800/60 bg-slate-950/60 p-4">
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-2">
                      <label className="flex flex-col gap-1 text-xs text-slate-300">
                        <span>{t("settings.retry.maxRetries.label")}</span>
                        <input
                          type="number"
                          min={0}
                          step={1}
                        ...
                        <p className="text-[11px] text-slate-500">
                          {t("settings.retry.maxRetries.hint")}
                        </p>
                      </label>
                      <label className="flex flex-col gap-1 text-xs text-slate-300">
                        <span>{t("settings.retry.initialDelayMs.label")}</span>
                        <input
                          type="number"
                          min={0}
                          step={50}
                        ...
                        />
                        <p className="text-[11px] text-slate-500">
                          {t("settings.retry.initialDelayMs.hint")}
                        </p>
                      </label>
                      <label className="flex flex-col gap-1 text-xs text-slate-300">
                        <span>{t("settings.retry.multiplier.label")}</span>
                        <input
                          type="number"
                          min={1}
                          step={0.1}
                        ...
                        />
                        <p className="text-[11px] text-slate-500">
                          {t("settings.retry.multiplier.hint")}
                        </p>
                      </label>
                      <label className="flex flex-col gap-1 text-xs text-slate-300">
                        <span>{t("settings.retry.maxDelayMs.label")}</span>
                        <input
                          type="number"
                        ...
                        />
                        <p className="text-[11px] text-slate-500">
                          {t("settings.retry.maxDelayMs.hint")}
                        </p>
                      </label>
                    </div>
                    <div className="mt-3 space-y-3 text-xs text-slate-300">
                      <div>
                        <label className="flex items-center justify-between gap-3">
                          <span>
                            {t("settings.retry.respectServerHints.label")}
                          </span>
                          <input
                            type="checkbox"
                            checked={retryPolicy.respectServerRetryAfter}
                            onChange={handleRetryToggleChange(
                        ...
                        <p className="mt-1 text-[11px] text-slate-500">
                          {t("settings.retry.respectServerHints.hint")}
                        </p>
                      </div>
                      <div>
                        <label className="flex items-center justify-between gap-3">
                          <span>{t("settings.retry.autoTune429.label")}</span>
                          <input
                            type="checkbox"
                        ...
                        <p className="mt-1 text-[11px] text-slate-500">
                          {t("settings.retry.autoTune429.hint")}
                        </p>
                      </div>
                    </div>
                  </div>
                  <p className="mt-2 text-[11px] text-slate-500">
                    API 키는 로컬 장치에 암호화되지 않은 상태로 저장되므로
                    보안에 유의하세요.
                  </p>
                </div>
              );
            ...
            {(apiKeyMessage || apiKeyError) && (
              <div className="space-y-1 text-xs">
                {apiKeyMessage && (
                  <p className="text-slate-400">{apiKeyMessage}</p>
                )}
        ...
        <section className="rounded-2xl border border-slate-800/60 bg-slate-900/60 p-6 shadow-inner shadow-black/30">
          <h3 className="text-lg font-semibold text-white">
            {t("settings.steam.title")}
          </h3>
          <div className="mt-4 space-y-4">
            <label className="block text-sm font-medium text-slate-300">
              {t("settings.steam.pathLabel")}
            </label>
            <input
              type="text"
              value={explicitPath}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {isScanning
                  ? t("settings.steam.scanning")
                  : t("settings.steam.scan")}
              </button>
            </div>
            {libraryError && (
        ...
        <section className="rounded-2xl border border-slate-800/60 bg-slate-900/60 p-6 shadow-inner shadow-black/30">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <label className="flex flex-col gap-2 text-sm text-slate-300">
              <span>{t("settings.limits.concurrency")}</span>
              <input
                type="number"
                value={concurrency}
                min={1}
              ...
              <p className="text-xs text-slate-500">
                {t("settings.limits.hints.concurrency")}
              </p>
            </label>
            <label className="flex flex-col gap-2 text-sm text-slate-300">
              <span>{t("settings.limits.workers")}</span>
              <input
                type="number"
                value={workerCount}
                min={1}
              ...
              />
              <p className="text-xs text-slate-500">
                {t("settings.limits.hints.workers")}
              </p>
            </label>
            <label className="flex flex-col gap-2 text-sm text-slate-300">
              <span>{t("settings.limits.bucket")}</span>
              <input
                type="number"
                value={bucketSize}
                min={1}
              ...
              />
              <p className="text-xs text-slate-500">
                {t("settings.limits.hints.bucket")}
              </p>
            </label>
            <label className="flex flex-col gap-2 text-sm text-slate-300">
              <span>{t("settings.limits.refillMs")}</span>
              <input
                type="number"
                value={refillMs}
                min={100}
              ...
              />
              <p className="text-xs text-slate-500">
                {t("settings.limits.hints.refillMs")}
              </p>
            </label>
            <label className="md:col-span-2 xl:col-span-4 flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-sm text-slate-300">
              <span>{t("settings.limits.autoTune")}</span>
              <input
                type="checkbox"
                checked={autoTuneConcurrencyOn429}
                onChange={(event) =>
                  setAutoTuneConcurrencyOn429(event.target.checked)
                }
                className="h-4 w-4 rounded border-slate-700 bg-slate-900"
              />
            </label>
        ...
        <section className="rounded-2xl border border-slate-800/60 bg-slate-900/60 p-6 shadow-inner shadow-black/30">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-xs text-slate-300 sm:grid-cols-3">
                      <label className="flex flex-col gap-1">
                        ...
                        <input
                          ...
                          onChange={handleRetryPolicyNumberChange(
                            provider.id,
                            "maxAttempts",
                          )}
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              checked={checked}
                              onChange={() =>
                                handleRetryableErrorToggle(provider.id, code)
                              }
          ...
          <div className="mt-4 space-y-2 text-sm text-slate-300">
            ...
            <p className="text-xs text-slate-500">
              {t("settings.limits.hints.autoTune")}
            </p>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-800/60 bg-slate-900/60 p-6 shadow-inner shadow-black/30">
          ...
          <div className="mt-4 space-y-4 text-sm text-slate-300">
            <label className="flex items-center justify-between gap-3">
              <span>{t("settings.rules.backendLogging")}</span>
              <input
                type="checkbox"
                checked={enableBackendLogging}
                onChange={(event) =>
                  setEnableBackendLogging(event.target.checked)
                }
                className="h-4 w-4 rounded border-slate-700 bg-slate-900"
              />
            </label>
            <label className="flex items-center justify-between gap-3">
              <span>{t("settings.rules.serverHints")}</span>
              <input
                type="checkbox"
            ...
            <label className="flex items-center justify-between gap-3">
              <span>{t("settings.rules.placeholderGuard")}</span>
              <input
                type="checkbox"
                checked={enforcePlaceholderGuard}
                onChange={(event) =>
                  setEnforcePlaceholderGuard(event.target.checked)
                }
                className="h-4 w-4 rounded border-slate-700 bg-slate-900"
              />
            </label>
            <label className="flex items-center justify-between gap-3">
              <span>{t("settings.rules.validationMode")}</span>
              <select
              ...
              >
                <option value="strict">
                  {t("settings.rules.validationModes.strict")}
                </option>
                <option value="relaxed_xml">
                  {t("settings.rules.validationModes.relaxed_xml")}
                </option>
              </select>
            </label>
            <p className="text-xs text-slate-400">
              {t("settings.rules.validationModeDesc")}
            </p>
            <label className="flex items-center justify-between gap-3">
              <span>{t("settings.rules.dllResources")}</span>
              <input
                type="checkbox"
                checked={prioritizeDllResources}
                onChange={(event) =>
                  setPrioritizeDllResources(event.target.checked)
                }
                className="h-4 w-4 rounded border-slate-700 bg-slate-900"
              />
            </label>
            <label className="flex items-center justify-between gap-3">
              <span>{t("settings.rules.qualitySampling")}</span>
              <input
                type="checkbox"
                checked={enableQualitySampling}
                onChange={(event) =>
                  setEnableQualitySampling(event.target.checked)
                }
                className="h-4 w-4 rounded border-slate-700 bg-slate-900"
              />
            </label>
